...
class AliPayResultAPIView(APIView):
    ...
    def get(self, request):
        ...
        # 验证alipay的异步通知，data来自支付宝回调post 字典格式
        data = request.query_params.dict()
        # 获取签名信息
        signture = data.pop("sign")

        success = alipay.verify(data, signture)
        success = True
        if success:
            # 支付成功后
    ...
    #         order = Order.objects.get(order_number=order_number, order_status=0)
    #     except Order.DoesNotExist:
    #         return Response({"message": "对不起支付结果查询失败"}, status=status.HTTP_400_BAD_REQUEST)
    #
    #     with transaction.atomic():
    #         savepoint = transaction.savepoint()
    #
    ...
    #
    #         except:
    #             log.error("订单处理过程中的问题")
    #             transaction.savepoint_rollback(savepoint)
    #             return Response({"message": "更新订单相关信息失败"}, status=status.HTTP_400_BAD_REQUEST)
    #
    #     return Response({"message": "支付成功",
    #                      "success": "success",
    #                      "pay_time": order.pay_time,
